...
{
    public abstract class PushMsgBP
    {
        public static void PushMsgActioning(string TaskID)
        {
            ...
            //  Log King
            //CommonDA.InsertEventHistory("取得TASK开始");

            DataSet dsTaskInfo = new DataSet();
            dsTaskInfo = PushMsgDA.GetPushMsgTaskInfo(TaskID);

            if (dsTaskInfo.Tables.Count == 0 || dsTaskInfo.Tables[0].Rows.Count == 0)
            {
                return;
            }

            //  Log King
            //CommonDA.InsertEventHistory("取得TASK完成");

            //  Log King
            //CommonDA.InsertEventHistory("取得Push开始");

            DataSet dsPushMsg = new DataSet();
            dsPushMsg = PushMsgDA.GetPushMsgList(TaskID, dsTaskInfo.Tables[0].Rows[0]["Type"].ToString(), dsTaskInfo.Tables[0].Rows[0]["Push_Users"].ToString());
            ...
            if (dsPushMsg.Tables.Count > 0 && dsPushMsg.Tables[0].Rows.Count > 0)
            {
                ...
                //PushHelp.PusMsgHelping(dsPushMsg, TaskID, dsTaskInfo.Tables[0].Rows[0]["Push_Content"].ToString());

                string strHead = "{\"pushDynamicList\":[";
                string strFoot = "],\"title\":\"" + dsTaskInfo.Tables[0].Rows[0]["Push_Title"].ToString() + "\",\"content\":\"" + dsTaskInfo.Tables[0].Rows[0]["Push_Content"].ToString() + "\",\"objType\":" + dsTaskInfo.Tables[0].Rows[0]["PushType"].ToString() + ",\"objUrl\":\"" + dsTaskInfo.Tables[0].Rows[0]["Wap_Url"].ToString() + "\",\"pic\":\"\",\"isPopup\":\"\",\"taskId\":\"" + TaskID + "\",\"endTime\":\"\"}";
                string objLinkId = ("8".Equals(dsTaskInfo.Tables[0].Rows[0]["PushType"].ToString())) ? dsTaskInfo.Tables[0].Rows[0]["PackageCode"].ToString() : TaskID;
                try
                {
            ...
            // Update Status
            PushMsgDA.UpdateSendPushStatus(TaskID, "3", "2");

            if ("2".Equals(dsTaskInfo.Tables[0].Rows[0]["Type"].ToString().Trim()))
            {
                PushMsgDA.ClearTempPushTelPhone(TaskID);
            }
            //  Log King
            //CommonDA.InsertEventHistory("更新状态 清空数据完成");
