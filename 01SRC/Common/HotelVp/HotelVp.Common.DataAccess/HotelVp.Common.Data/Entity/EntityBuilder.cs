		...
		private static void DoFillEntity(IEntityDataSource ds, Object obj, Type type, string prefix)
		{
			...
			foreach (string columnName in ds)
			{
				...
				if (ds[columnName] != DBNull.Value && ValidateData(propertyBindingInfo, ds[columnName]))
				{
                    ...
                    Type propertyType = propertyBindingInfo.PropertyInfo.PropertyType;

                    if (propertyType == typeof(string))
					{
						val = val.ToString().Trim();
                    }
                    else if (propertyType.FullName.StartsWith("System.Nullable`1"))
                    {
                        Type enumType = propertyType.GetProperty("Value").PropertyType;

                        if (val != null && enumType.IsEnum)
                        {
                            ConstructorInfo constructorInfo = propertyType.GetConstructor(new Type[] { enumType });

                            val = constructorInfo.Invoke(new object[] { val });
                        }
                    }
